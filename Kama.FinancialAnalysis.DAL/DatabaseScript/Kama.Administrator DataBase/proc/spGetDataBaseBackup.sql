...
GO
--گرفتن بک آپ دیتابیس ها که در ماه اخیر گرفته شده است
CREATE OR ALTER PROC alg.spGetDataBaseBackup
	@DataBaseName VARCHAR(200), 
	@BackupType   VARCHAR(200) --Transaction Log Backup,Differential database backup Backup
AS
     WITH Backups AS 
	  (
        ...
        WHERE recovery_model IN('FULL')
            AND bs.backup_start_date > DATEADD(month, -1, SYSDATETIME()) --only look at last two months
